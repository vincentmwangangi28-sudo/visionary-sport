...
export const SocialShareButtons = ({ 
  ...
  const shareMessage = prediction 
    ? `🎯 ${title}\n\n${text}\n\n🔮 Prediction: ${prediction}${confidence ? ` (${confidence}% confidence)` : ''}\n\n🏆 Get AI predictions at PredictPro Guru!`
    : `${title}\n\n${text}`;
  ...
  const handleShare = (platform: string) => {
    trackFeatureEngagement('social_share', platform);
    
    // GA4 event tracking for social shares
    if (typeof window !== 'undefined' && (window as any).gtag) {
      (window as any).gtag('event', 'share_prediction', {
  ...
  const handleNativeShare = async () => {
    if (navigator.share) {
      try {
        ...
        trackFeatureEngagement('social_share', 'native');
      } catch (err) {
