...
const LAST_SYNC_KEY = 'predictpro_last_offline_sync_v5';
...
export async function prewarmOfflineCaches(): Promise<void> {
  ...
  if ('caches' in window) {
    try {
      const imgCache = await caches.open('predictpro-images-v5');
      ...
      const dataCache = await caches.open('predictpro-data-v5');
