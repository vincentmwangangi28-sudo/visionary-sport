...
interface WavaveExpressRequest {
  phone: string;
  amount: number;
...
serve(async (req) => {
  ...
  try {
    ...
    if (!WAVAVE_API_KEY || !WAVAVE_MERCHANT_CODE) {
      return new Response(
        JSON.stringify({ success: false, error: 'Wavave Express not configured' }),
    ...
    const body: WavaveExpressRequest = await req.json();
    ...
    if (!wavaveResponse.ok || !wavaveData.success) {
      console.error('Wavave error:', wavaveData);
      return new Response(
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: 'Wavave Express initiated. You will receive a push notification on your phone.',
        data: {
    ...
    console.error('Error in wavave-express:', error);
    return new Response(
